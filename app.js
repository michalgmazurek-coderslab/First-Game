document.addEventListener("DOMContentLoaded", function() {
	...
	var addScoreBtn = document.getElementById("addScoreBtn");

    addScoreBtn.addEventListener("click", function(event) {
        event.preventDefault();
        var userNameInputValue = document.getElementById("nameInput").value;  // $("#nameInput").val();
        ...
        firebase.database().ref().push({
            ...
            var topScores = firebase.database().ref().orderByChild('score').limitToLast(10).on('value', function(fbdata) {
                ...
                var myResults = [];
                var yourScoreSection = document.querySelector(".yourScore");
                var topTenSection = document.getElementById("topTenSection");
                var topTenScores = document.getElementById("topTenScores");
                ...
                yourScoreSection.style.display = "none";
                topTenSection.style.display = "flex";

                for (var i = 0; i < sortedObjectsArray.length; i++) {
                    var newLi = document.createElement("li");
                    topTenScores.appendChild(newLi);
                    newLi.innerHTML = (i + 1) + ".&nbsp;" + sortedObjectsArray[i].userName + "&nbsp;:&nbsp;" + sortedObjectsArray[i].score;
                }
            });
        });
    });
